...
<div x-data
     ...
     @click.self="$store.commandPalette.open = false"
    ...
    <div class="command-palette w-full max-w-lg">
        <div class="p-3 border-b border-white/10 flex items-center gap-3">
            ...
            <input type="text"
                   ...
                   class="flex-1 bg-transparent text-white placeholder:text-slate-500 focus:outline-none text-sm"
                   placeholder="Rechercher une page ou une action..."
                   x-init="$watch('$store.commandPalette.open', v => v && $nextTick(() => $el.focus()))">
        ...
        <div class="py-2 max-h-80 overflow-y-auto">
            <template x-for="(cmd, index) in $store.commandPalette.filtered" :key="cmd.url">
                <a :href="cmd.url" @click="$store.commandPalette.open = false"
                   class="command-item"
                   :class="{ 'selected': index === $store.commandPalette.selected }"
                   @mouseenter="$store.commandPalette.selected = index">
            ...
            <div x-show="$store.commandPalette.filtered.length === 0"
                 class="px-4 py-8 text-center text-slate-500 text-sm">Aucun résultat</div>
        </div>
...
<aside class="sidebar hidden lg:flex w-60 bg-black/40 border-r border-white/[0.06]" style="backdrop-filter: blur(20px);">
    ...
    <div class="flex items-center gap-3 px-5 h-16 border-b border-white/[0.06] flex-shrink-0">
        <div class="w-8 h-8 rounded-xl flex items-center justify-center font-black text-sm text-white"
             style="background: linear-gradient(135deg, #3b82f6, #8b5cf6)">S</div>
        <div>
    ...
    <nav class="flex-1 px-3 py-4 overflow-y-auto space-y-0.5">
        ...
        <a href="{{ route('budget.index') }}" class="nav-link {{ request()->routeIs('budget.index') ? 'active' : '' }}">
            <span class="text-base">💰</span><span>Vue d'ensemble</span>
        </a>
        ...
        <a href="{{ route('portfolio.dividends') }}" class="nav-link {{ request()->routeIs('portfolio.dividends') ? 'active' : '' }}">
            <span class="text-base">💵</span><span>Dividendes</span>
        </a>
        <a href="{{ route('portfolio.analysis') }}" class="nav-link {{ request()->routeIs('portfolio.analysis') ? 'active' : '' }}">
            <span class="text-base">🔬</span><span>Analyse tech.</span>
        </a>

        <div class="sidebar-label">Espace</div>
        <a href="{{ route('team.settings') }}" class="nav-link {{ request()->routeIs('team.settings') ? 'active' : '' }}">
            <span class="text-base">👥</span><span>Mon espace</span>
        </a>
    </nav>
    ...
    <div class="px-3 py-3 border-t border-white/[0.06] flex-shrink-0">
        ...
        <div class="mb-2 px-2 py-1.5 bg-white/[0.03] rounded-lg">
            <div class="text-[10px] text-slate-600 uppercase tracking-widest mb-0.5">Espace actif</div>
        ...
        <div class="flex items-center gap-2 px-2 py-1.5 group">
            ...
            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <form method="POST" action="{{ route('logout') }}" class="inline">
                    @csrf
                    <button class="text-slate-500 hover:text-red-400 transition-colors text-sm" title="Déconnexion">⏻</button>
                </form>
            </div>
        </div>
    </div>
...
<!-- ── Mobile header ───────────────────────────────────────── -->
<div class="lg:hidden fixed top-0 inset-x-0 z-20 h-14 flex items-center px-4 justify-between border-b border-white/[0.06]"
     style="background: rgba(2,8,23,0.9); backdrop-filter: blur(20px);">
    ...
    <div class="flex items-center gap-2" x-data="{ open: false }">
        <button @click="$store.commandPalette.toggle()" class="text-slate-400 hover:text-white p-2" title="⌘K">⌘</button>
        ...
        <div x-show="open" @click.away="open = false"
             class="absolute right-4 top-14 w-56 py-2 command-palette z-50">
            <a href="{{ route('dashboard') }}" class="command-item">📊 Dashboard</a>
            <a href="{{ route('budget.index') }}" class="command-item">💰 Budget</a>
            <a href="{{ route('budget.transactions') }}" class="command-item">💸 Transactions</a>
            <a href="{{ route('budget.goals') }}" class="command-item">🎯 Objectifs</a>
            <a href="{{ route('import.index') }}" class="command-item">📂 Importer</a>
            <a href="{{ route('portfolio.index') }}" class="command-item">📊 Portefeuille</a>
            <a href="{{ route('portfolio.positions') }}" class="command-item">📉 Positions</a>
            <a href="{{ route('portfolio.alerts') }}" class="command-item">🔔 Alertes</a>
            <a href="{{ route('portfolio.dividends') }}" class="command-item">💵 Dividendes</a>
            <a href="{{ route('team.settings') }}" class="command-item">👥 Mon espace</a>
            <div class="border-t border-white/10 mt-1 pt-1">
                <form method="POST" action="{{ route('logout') }}" class="px-4 py-1">
                    @csrf
                    <button class="text-red-400 text-sm w-full text-left">⏻ Déconnexion</button>
                </form>
            </div>
        </div>
    </div>
...
<div class="lg:pl-60 min-h-screen flex flex-col">
    <!-- Page header -->
    <header class="hidden lg:flex items-center justify-between h-16 px-8 border-b border-white/[0.05]"
            style="background: rgba(2,8,23,0.6); backdrop-filter: blur(12px);">
        <div>
        ...
        <div class="flex items-center gap-3">
            @yield('actions')
            <button @click="$store.commandPalette.toggle()"
                    class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs text-slate-500 hover:text-slate-300 transition-colors"
    ...
    @if(session('success'))
    <script>document.addEventListener('alpine:init', () => setTimeout(() => window.toast?.success('{{ addslashes(session('success')) }}'), 100));<\/script>
    @endif
    @if(session('error'))
    <script>document.addEventListener('alpine:init', () => setTimeout(() => window.toast?.error('{{ addslashes(session('error')) }}'), 100));<\/script>
    @endif
    ...
    <footer class="px-8 py-3 border-t border-white/[0.04] flex items-center justify-between text-xs text-slate-600">
        <span>Shadow Finance · Laravel 12 · CAD</span>
        <span>{{ now()->setTimezone('America/Toronto')->format('d M Y · H:i') }}</span>
    </footer>
</div>
